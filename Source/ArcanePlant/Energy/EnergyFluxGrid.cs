...
namespace VVRace
{
    public struct EnergyFluxGridCell
    {
        ...
        public EnergyFluxGridCell WithArcanePlant(ArcanePlant arcanePlant)
        {
            return new EnergyFluxGridCell
            {
                transmitter = transmitter,
        ...
        public void ChangeFluxNetwork(EnergyFluxNetwork network)
        {
            if (arcanePlant != null)
            {
                if (arcanePlant.EnergyFluxNetwork != null)
                {
                    arcanePlant.EnergyFluxNetwork.RemoveEnergyNode(arcanePlant);
                }

                network.AddEnergyNode(arcanePlant, arcanePlant.EnergyFluxNode);
            }
        }
    }
    ...
    public class EnergyFluxGrid
    {
        ...
        public void AddEnergyAcceptor(EnergyAcceptor energyAcceptor)
        {
            ...
            if (energyAcceptor is ArcanePlant arcanePlant)
            {
                _energyFluxGridCells[pos] = gridCell.WithArcanePlant(arcanePlant);
            }
        ...
        public void RemoveEnergyAcceptor(EnergyAcceptor energyAcceptor)
        {
            ...
            if (_energyFluxGridCells.TryGetValue(pos, out var gridCell))
            {
                if (energyAcceptor is ArcanePlant)
                {
                    _energyFluxGridCells[pos] = gridCell.WithArcanePlant(null);
                }
        ...
        public void RefreshNetworks()
        {
            ...
            while (remainingGridCells.Count > 0)
            {
                ...
                foreach (var cell in connectedSet)
                {
                    ...
                    {
                        gridCell.arcanePlant.EnergyFluxNetwork = network;
                        network.AddEnergyNode(gridCell.arcanePlant, gridCell.arcanePlant.EnergyFluxNode);
                    }
